...
const fetchProducts = async () => {
  try {
    const response = await fetch("http://localhost:3000/api/products?category=deals", { cache: 'no-store' }, {
      method: "GET"
    });
...
const DealsCategory = async () => {
  const products = await fetchProducts();
  return (
    <>
    <h1  className=' text-3xl mx-auto text-center font-semibold '>Deals Products</h1>
